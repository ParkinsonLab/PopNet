        ...
        for chrName in matrixDict:
            if re.search("(?i)chr", chrName): #Only real chromosomes allowed. 
                ...
                for index in chrBranch:
                    ...
                    dimension = len(indexBranch.keys()) #dimensions match the number of samples
...
#                     print "hit zzz", matrixStats[entry[2]];  
        ...
        sortedInvStats = sorted({(v,k) for k, v in matrixStats.iteritems()}, reverse=True)
...
def toRange(numList):
        ...
        return [(t[0][1], t[-1][1]) for t in (tuple(g[1]) for g in itertools.groupby(enumerate(sorted(numList)), lambda(i, x): i - x))]
