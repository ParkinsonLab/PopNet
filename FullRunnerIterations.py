...
#     baseDirectory = '/data/new/javi/plasmo/pipeline/TEST'
    ...
    baseDirectory = '/scratch/j/jparkins/xescape/yeast/pipeline'
    baseOutputDirectory = baseDirectory + '/matrix'
    rawresultpath = baseOutputDirectory + "results.txt"
    densitypath = baseOutputDirectory + "density.txt"
    countpath = baseOutputDirectory + "counted.txt"  
    grouppath = baseOutputDirectory + "groups.txt"
    
    if not isdir(baseOutputDirectory): os.mkdir(baseOutputDirectory)
...
#####################################################################################################   
    ...
    while(x>=1):
        ...
        print('Running Iteration with iValue = {0:d}, piValue = {1:d}'.format(iValue, piValue))
    
        outputDirectory = baseDirectory + '/matrix/' + str(x)
        x -= 1
